...
definition edges 
  where "edges p \<equiv> {e. e edge_of p}"
definition vertices
  where "vertices p \<equiv> {v. v face_of p \<and> aff_dim v = 0}"
definition faces 
  where "faces p \<equiv> {v. v face_of p \<and> aff_dim v = 2}"
...
proof -
  ...
  hence "(s,m) \<in> {3,4,5} \<times> {3,4,5}" (is "(s,m) \<in> ?X") by auto
  hence "(s,m) \<in> ?X" by simp
